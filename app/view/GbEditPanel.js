Ext.define('Gb41.view.GbEditPanel', {
	extend: 'Ext.form.Panel',
	alias: 'widget.gbeditpanel',
		requires: [
			'Ext.form.field.File',
			'Ext.form.FieldSet',
	...
	initComponent: function () {
		...
		var editLayersConfig = edit_configs[GbZh.base.ViewerState.getActiveTopic()];

		if (!Ext.isArray(editLayersConfig)) {
			alert('editLayer configuration missing. Can\'t edit');
			return;
		}

		this.editLayers = editLayersConfig;
		...
		this.add([
			...
			this.yCoordInput,
			this.xyAddButton,
			this.arcIntersectionInputs[0],
			this.arcIntersectionInputs[1],
	...
	onActivate: function() {
		if (this.editLayer != null) {
			...
			GbZh.base.ViewerState.fireEvent('activateolcontrols', 'GbMapController', 'none');
		}
	},
	...
	*/
	...
	addSnappingLayer: function(config) {
		...
		if (config.gpx == null) {
			...
			this.snappingTooltip = Ext.create('Ext.tip.ToolTip', {
				...
				html: "Gewähltes Fangthema ist nur für Massstäbe bis zu 1:" + this.snappingMaxScale + " aktiv."
			});
			this.map.events.on({
	...
	uploadGpx: function() {
		...
		if (form.isValid()) {
			this.el.mask("Upload der GPX Datei..."); // FIXME: remove if waitMsg works again
			form.submit({
				url: this.uploadGpxUrl,
	//                waitMsg: "Uploading GPX file...", // FIXME: reenable when http://www.sencha.com/forum/showthread.php?228970 is fixed
				success: function(form, action) {
					// close upload panel
					this.gpxButton.toggle(false);
					this.gpxButton.handler.call(this.gpxButton.scope);
					this.el.unmask(); // FIXME: remove if waitMsg works again
					...
					if (gpxFeatures.length > 0) {
						...
						Ext.Msg.show({
							title: "GPX Import",
							msg: "Konnte keine Objekte finden.<br/>Ist dies eine gültige GPX Datei?",
							buttons: Ext.Msg.OK,
				...
				failure: function(form, action) {
					...
					Ext.Msg.show({
						title: "GPX Import",
						msg: "GPX Import fehlgeschlagen:<p/>" + action.result.msg,
						buttons: Ext.Msg.OK,
						icon: Ext.MessageBox.ERROR
					});
				},
	...
	cancelFeatureEdit: function() {
		if (this.feature.state == OpenLayers.State.INSERT) {
			...
			// reset feature
			this.editLayer.drawFeature(this.feature, {display: "kein"});
