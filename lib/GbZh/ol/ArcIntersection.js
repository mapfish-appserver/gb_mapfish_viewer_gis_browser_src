...
OpenLayers.Control.ArcIntersection = OpenLayers.Class(OpenLayers.Control, {
  ...
  initialize: function (layer, options) {
    ...
    this.intersectionsSelectStyle = OpenLayers.Util.extend({}, this.layer.styleMap.createSymbolizer(null, "select"));
    options = options || {};
   ...
   */
  activate: function () {
    return (this.handlers.setCenter.activate() && this.handlers.intersectionFeature.activate() && OpenLayers.Control.prototype.activate.apply(this, arguments));
  },
  /**
   ...
   */
  deactivate: function () {
    var deactivated = (this.handlers.setCenter.deactivate() && this.handlers.intersectionFeature.deactivate() && OpenLayers.Control.prototype.deactivate.apply(this, arguments));
    if (deactivated) {
      // reset features
  ...
  updateCircles: function () {
    ...
    for (var i = 0; i < this.circles.length; i++) {
      ...
      if (circle.center != null && circle.radius != null) {
        ...
        new OpenLayers.Geometry.Polygon.createRegularPolygon(circle.center, circle.radius, 120, 0), null, this.circlesStyle);
        // mark as temporary
        circleFeature._sketch = true;
        this.virtualFeatures.push(circleFeature);
        var centerFeature = new OpenLayers.Feature.Vector(
        circle.center, null, this.circlesStyle);
        // mark as temporary
