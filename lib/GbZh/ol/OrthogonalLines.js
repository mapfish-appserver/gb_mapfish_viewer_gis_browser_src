...
OpenLayers.Control.OrthogonalLines = OpenLayers.Class(OpenLayers.Control, {
  ...
  initialize: function (layer, options) {
    ...
    this.orthogonalLinesSelectStyle = OpenLayers.Util.extend({}, this.layer.styleMap.createSymbolizer(null, "select"));
    options = options || {};
   ...
   */
  activate: function () {
    return (this.handlers.setBasePoint.activate() && this.handlers.orthogonalLinesFeature.activate() && OpenLayers.Control.prototype.activate.apply(this, arguments));
  },
  /**
   ...
   */
  deactivate: function () {
    var deactivated = this.handlers.setBasePoint.deactivate() && this.handlers.orthogonalLinesFeature.deactivate() && OpenLayers.Control.prototype.deactivate.apply(this, arguments);
    if (deactivated) {
      // reset temp features
      this.layer.destroyFeatures(this.virtualFeatures, {
        silent: true
      });
      this.virtualFeatures = [];
      // reset base points
  ...
  updateConstruction: function () {
    ...
    if (points.length >= 2) {
      ...
      new OpenLayers.Geometry.LineString(points), null, this.orthogonalLinesStyle);
      // mark as orthogonal lines
      orthogonalLinesFeature._orthogonalLines = true;
      this.virtualFeatures.push(orthogonalLinesFeature);
      this.layer.addFeatures([orthogonalLinesFeature], {
        silent: true
      });
    }
  },
  ...
  updateTempFeatures: function () {
    ...
    for (var i = 0; i < this.basePoints.length; i++) {
      ...
      this.basePoints[i], null, this.baseFeaturesStyle);
      // mark as temporary
    ...
    if (this.basePoints.length == 2) {
      ...
      new OpenLayers.Geometry.LineString(this.basePoints), null, this.baseFeaturesStyle);
      // mark as temporary
      baseLineFeature._sketch = true;
      this.virtualFeatures.push(baseLineFeature);
    }
    this.layer.addFeatures(this.virtualFeatures, {
      silent: true
    });
  },
  /**
